// Homepage hero banner, "Причины заказать уборку" carousel, and the
// "Почему выбирают нас" grid are DB-driven now (HomeContent / WhyUsPage
// models, /admin, /admin/why-us -- lib/homeContentData.ts, lib/whyUsPageData.ts).
...
// Homepage "Простой и понятный процесс" timeline, "Уборка с понятными
// условиями" block, and homepage FAQ are DB-driven now (HomeContent model —
// lib/homeContentData.ts).
